    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-xs">
          ...
          <p className="text-2xl font-bold text-slate-900 dark:text-slate-100 mt-2">{totalPurchasedSeats}</p>
          <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">Across all active pools</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-xs">
          ...
          <p className="text-2xl font-bold text-slate-900 dark:text-slate-100 mt-2">{totalAllocatedSeats}</p>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-xs">
          ...
          <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400 mt-2">{totalRemainingSeats}</p>
          <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">Ready to assign to members</p>
        </div>
      </div>
      ...
      {isPurchaseOpen && (
        <form onSubmit={handlePurchaseSubmit} className="bg-slate-50 dark:bg-slate-800/80 border border-blue-200 dark:border-blue-900/50 rounded-xl p-5 space-y-4 animate-in fade-in">
          <div className="flex items-center justify-between">
            <h5 className="text-xs font-bold text-blue-900 dark:text-blue-300 uppercase tracking-wider">Purchase License Seat Package</h5>
            <span className="text-xs font-semibold text-blue-600 dark:text-blue-400">${totalPrice} total</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              <label className="block text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1">Plan Tier</label>
              <select
                value={selectedPlan}
                onChange={(e) => setSelectedPlan(e.target.value as api.LicensePlan)}
                className="w-full text-xs p-2 border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100"
              >
            ...
            <div>
              <label className="block text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1">Seat Quantity</label>
              <input
                type="number"
                min="1"
                max="500"
            ...
            <div>
              <label className="block text-[11px] font-semibold text-slate-700 dark:text-slate-300 mb-1">Billing Interval</label>
              <div className="grid grid-cols-2 gap-1 bg-slate-200 dark:bg-slate-900 p-0.5 rounded-lg text-xs font-semibold">
                <button
                  type="button"
                  onClick={() => setBillingInterval('MONTHLY')}
                  className={`py-1.5 rounded-md text-center cursor-pointer transition-colors ${
                    billingInterval === 'MONTHLY' ? 'bg-white text-slate-900 shadow-xs dark:bg-slate-700 dark:text-white' : 'text-slate-600 dark:text-slate-400'
                  }`}
                >
                  Monthly
                </button>
                <button
                  ...
                  className={`py-1.5 rounded-md text-center cursor-pointer transition-colors ${
                    billingInterval === 'YEARLY' ? 'bg-white text-slate-900 shadow-xs dark:bg-slate-700 dark:text-white' : 'text-slate-600 dark:text-slate-400'
                  }`}
                >
                  Yearly (Save 17%)
                </button>
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={6} className="p-6 text-center text-slate-500 dark:text-slate-400">
                  <p className="font-semibold text-slate-700 dark:text-slate-200">No active license pools</p>
              ...
              activePools.map((pool) => (
                <tr key={pool.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                  <td className="p-3 pl-4 font-bold text-slate-900 dark:text-slate-100 flex items-center gap-1.5">
                    <Sparkles className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />
                    {pool.planType}
                  </td>
                  <td className="p-3">
                    <span className="font-semibold text-slate-900 dark:text-slate-100">{pool.allocatedSeats}</span> / {pool.totalSeats} seats
                  </td>
                  <td className="p-3">
                    <span className="font-bold text-emerald-600 dark:text-emerald-400">{pool.remainingSeats} seats</span>
                  </td>
                  <td className="p-3 text-slate-600 dark:text-slate-300">{pool.billingInterval}</td>
                  <td className="p-3 text-slate-600 dark:text-slate-300">
                    {pool.validUntil ? new Date(pool.validUntil).toLocaleDateString() : 'Active'}
                  </td>
                  <td className="p-3 text-right pr-4">
                    <span className="inline-flex items-center gap-1 bg-green-50 text-green-700 border border-green-200 px-2 py-0.5 rounded-full text-[11px] font-semibold dark:bg-green-950/60 dark:text-green-300 dark:border-green-900/50">
      ...
      <div className="border border-slate-200 dark:border-slate-800 rounded-xl p-5 bg-white dark:bg-slate-900 space-y-4 shadow-xs">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="w-full sm:w-1/2">
            <label className="block text-[11px] font-semibold text-slate-600 dark:text-slate-300 mb-1">Target License Pool</label>
            <select
              value={assignPoolId}
          ...
          <div className="w-full sm:w-1/2">
            <label className="block text-[11px] font-semibold text-slate-600 dark:text-slate-300 mb-1">Select Member</label>
            <select
              value={assignUserId}
            ...
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {assignedMembers.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="p-4 text-center text-slate-400 dark:text-slate-500 italic">
                  ...
                  assignedMembers.map((m) => {
                    ...
                    return (
                      <tr key={m.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50">
                        <td className="p-2.5 pl-3 font-medium text-slate-900 dark:text-slate-100">
                          {m.email} {m.firstName && <span className="text-slate-400 dark:text-slate-500">({m.firstName} {m.lastName})</span>}
                        </td>
                        <td className="p-2.5">
                          <span className="inline-flex items-center gap-1 bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded-md text-[10px] font-semibold dark:bg-blue-950/60 dark:text-blue-300 dark:border-blue-900/50">
                        ...
                        <td className="p-2.5 text-right pr-3">
                          <button
                            ...
                            className="px-2.5 py-1 text-[11px] font-medium text-slate-600 hover:text-red-700 hover:bg-red-50 dark:text-slate-400 dark:hover:text-red-400 dark:hover:bg-red-950/50 rounded-md border border-slate-200 dark:border-slate-700 transition-colors cursor-pointer"
                          >
                            Unassign
                          </button>
                        </td>
