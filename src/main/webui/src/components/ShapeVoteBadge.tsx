...
function ShapeVoteItem({
  ...
  const hasUserVotedOnShape = votes.some((v) => v.userId === currentUserId);
  const isDuplicateDisallowed = votingConfig.allowDuplicateVotes === false && hasUserVotedOnShape;
  ...
  const handleCategorySelect = (e: React.MouseEvent, categoryId: string) => {
    e.stopPropagation();
    if (isLocked || !canEdit || isQuotaExhausted || isDuplicateDisallowed) return;
  ...
  // Do not render anything if shape has no votes
  ...
  return (
    <div
      ...
      className="pointer-events-auto z-20 flex items-center select-none"
        ...
        <div className="flex items-center gap-1">
          ...
          {canEdit && !isLocked && (
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                if (isQuotaExhausted || isDuplicateDisallowed) return;
                setIsOpen(true);
              }}
              disabled={isQuotaExhausted || isDuplicateDisallowed}
              title={
